...
public class FileLiveLogger implements ILogger {
    ...
    public void open() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD && SDCardUtil.detectAvailable()) {
            if (context != null) {
                ...
                String sdDir = cache.getAbsolutePath() + File.separator + logsDirName;   //播放SDK的log存储目录
                //create logs dir
                File logDir = new File(sdDir);
                if (!logDir.exists()) {
                    logDir.mkdirs();
                }
    ...
    public void v(String tag, String format, Object... args) {
        println(VERBOSE, tag, PLogger.buildMessage(format, args));
    }

    @Override
    public void w(String tag, String format, Object... args) {
        println(WARN, tag, PLogger.buildMessage(format, args));
    }

    @Override
    public void d(String tag, String format, Object... args) {
        println(DEBUG, tag, PLogger.buildMessage(format, args));
    }

    @Override
    public void e(String tag, String format, Object... args) {
        println(ERROR, tag, PLogger.buildMessage(format, args));
    }

    @Override
    public void i(String tag, String format, Object... args) {
        println(INFO, tag, PLogger.buildMessage(format, args));
    }
