package com.suning.baseui.utlis.threedes;
...
public class ThreeDESUtil {
    ...
    public static String Encode(String param, int keyIndex) throws Exception {
        ...
        String strDesEnc = new String(Base64.encode(output), CODINGTYPE);
        return strDesEnc;
    }
    ...
    public static String Decrypt(String strTobeDeCrypted, String strKey, String iv)
            throws Exception {
        byte byteIV[] = hex2byte(iv);
        byte input[] = base64Decode(strTobeDeCrypted);
        Key k = keyGenerator(strKey);
        IvParameterSpec ivSpec = byteIV.length != 0 ? ivGenerator(byteIV) : ivGenerator(DEFAULTIV);
        Cipher c = Cipher.getInstance(CRYPTALGORITHM);
    ...
    public static String Encrypt(String strTobeDeCrypted, String strKey, String iv) throws Exception {
        byte byteIV[] = hex2byte(iv);
        byte[] var3 = strTobeDeCrypted.getBytes(CODINGTYPE);
        Key k = keyGenerator(strKey);
        IvParameterSpec ivSpec = byteIV.length != 0 ? ivGenerator(byteIV) : ivGenerator(DEFAULTIV);
        Cipher c = Cipher.getInstance(CRYPTALGORITHM);
        c.init(1, k, ivSpec);
        byte[] output = c.doFinal(var3);
        return new String(Base64.decode(output), CODINGTYPE);
    }
